...
uint8_t   atsha204_read_conf(int fd, int slot, uint8_t *read_conf)
{
	...
	//printf("******************************\nslot:%d   config  :0x%x,0x%x\n******************************\n",slot,read_conf[0],read_conf[1]);
	return  1;
...
uint8_t atsha204_write_conf(int fd, int slot, uint8_t conf1, uint8_t conf2)
{
	...
	if(flag == 0 ){
		...
		config_params[0] = conf1;
		config_params[1] = conf2;
		config_params[2] = read_conf[0];
		config_params[3] =  read_conf[1];
	}else{
		atsha204_read_conf(fd, slot-1,read_conf);
		config_params[2] = conf1;
		config_params[3] = conf2;
...
void do_test()
{
	...
	atsha204_read_conf( chip, 0,tmp_conf);
	printf("slot 0  config  :0x%x,0x%x\n",tmp_conf[0],tmp_conf[1]);
	atsha204_read_conf( chip, 1,tmp_conf);
	printf("slot 1  config  :0x%x,0x%x\n",tmp_conf[0],tmp_conf[1]);
	atsha204_write_conf(chip,0,0x33,0x44);
	atsha204_write_conf(chip,1,0x22,0x11);
	atsha204_read_conf( chip, 0,tmp_conf);
	printf("slot 0  config  :0x%x,0x%x\n",tmp_conf[0],tmp_conf[1]);
	atsha204_read_conf( chip, 1,tmp_conf);
	printf("slot 1  config  :0x%x,0x%x\n",tmp_conf[0],tmp_conf[1]);
...
static cmd_tbl_t cmd_sha204_sub[] = {
	...
	U_BOOT_CMD_MKENT(test, 0, 1, do_test, "", ""),
};
